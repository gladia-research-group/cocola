...
class MusdbContrastivePreprocessed(Dataset):
        ...
        for track in tqdm(tracks, desc="Preprocessing tracks"):
            ...
            for _ in range(2):
                ...
                for i in range(len(stems[0])):
                    ...
                    example_id = f"{original_track_name}_chunk{i}_comb{anchor_mix_id}_{positive_mix_id}_{drums_id}_shift{frame_offset}"
    ...
    def _load_file_paths(self) -> None:
        ...
        file_paths_df = pd.concat(map(lambda track: pd.DataFrame(
            [[track / "anchor.pt", track / "positive.pt", track / "drums.pt"]], columns=["anchor", "positive", "drums"]), tracks), ignore_index=True)
        return file_paths_df

    def __len__(self) -> int:
        return len(self.file_paths_df)

    def __getitem__(self, idx) -> Dict[Literal["anchor", "positive", "drums"], torch.Tensor]:
        track_row = self.file_paths_df.iloc[idx]
        anchor_path, positive_path, drums_path = track_row["anchor"], track_row["positive"], track_row["drums"]

        anchor = torch.load(anchor_path, map_location="cpu")
        positive = torch.load(positive_path, map_location="cpu")
        drums = torch.load(drums_path, map_location="cpu")
        
        item = {"anchor": anchor, "positive": positive, "drums": drums}
